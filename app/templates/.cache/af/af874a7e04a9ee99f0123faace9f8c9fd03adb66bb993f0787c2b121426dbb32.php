...
class __TwigTemplate_4389d6306a1e73e16c7e33db43417287c8c8e17a7741cf5f127a2aec166af2b7 extends Twig_Template
{
    ...
    <div class=\"col-md-12\">
      <div class=\"box box-primary\">
        ...
        if ((twig_get_attribute($this->env, $this->getSourceContext(), ($context["owner_user"] ?? null), "foto", array(), "array") == 1)) {
            ...
            echo twig_escape_filter($this->env, sprintf("%s%s%s", twig_get_attribute($this->env, $this->getSourceContext(), ($context["owner_user"] ?? null), "id_user", array(), "array"), ".", twig_get_attribute($this->env, $this->getSourceContext(), ($context["owner_user"] ?? null), "ext_foto", array(), "array")), "html", null, true);
            echo "\" width=\"128\">
                ";
        } else {
            // line 23
            ...
            <div class=\"form-group\">
              <label class=\"col-sm-2 control-label\">
                {% if owner_user['foto'] == 1 %}
                  <img style=\"border:1px solid #eaeaea;border-radius:5px;\" src=\"views/app/images/avatares/{{ '%s%s%s'|format(owner_user['id_user'],'.',owner_user['ext_foto']) }}\" width=\"128\">
                {% else %}
                  <img style=\"border:1px solid #eaeaea;border-radius:5px;\" src=\"//ssl.gstatic.com/accounts/ui/avatar_2x.png\" width=\"128\">
                {% endif %}
              </label>
              <label style=\"font-size:25px;margin-top:10px;\" class=\"col-sm-8\">{{ owner_user['name'] }}</label>
            </div>
            <hr>

            <div class=\"form-group\">
              <label class=\"col-sm-2 control-label\">Email</label>
              <label style=\"text-align:left\" class=\"col-sm-8 control-label\">: {{ owner_user['email'] }}</label>
            </div>

            <div class=\"form-group\">
              <label class=\"col-sm-2 control-label\">Cargo</label>
              <label style=\"text-align:left\" class=\"col-sm-8 control-label\">: {{ owner_user['cargo'] }}</label>
            </div>

            <div class=\"form-group\">
              <label class=\"col-sm-2 control-label\">Telefono</label>
              <label style=\"text-align:left\" class=\"col-sm-8 control-label\">: {{ owner_user['fono'] }}</label>
            </div>

            <div class=\"form-group\">
              <label class=\"col-sm-2 control-label\">Rol de acceso</label>
              <label style=\"text-align:left\" class=\"col-sm-8 control-label\">: {{ owner_user['rol'] ? \"Admin\":\"Usuario\" }}</label>
            </div>

            <div class=\"form-group\">
              <label class=\"col-sm-2 control-label\">Perfil Asignado</label>
              <label style=\"text-align:left\" class=\"col-sm-8 control-label\">: {{ owner_user['perfil'] }}</label>
            </div>

            <div class=\"form-group\">
              <label class=\"col-sm-2 control-label\">Status</label>
              <label style=\"text-align:left\" class=\"col-sm-8 control-label\">: {{ owner_user['estado'] ? \"Activo\":\"Desactivo\" }}</label>
            </div>
          </div><!-- /.box body -->
...
{% endblock %}
", "portal/perfil_usuario.twig", "C:\\xampp\\htdocs\\proyectos\\login\\app\\templates\\portal\\perfil_usuario.twig");
    }
}
