    ...
    */
    ...
    public function modificar_ausencia() : array {
        ...
        try
        {
            ...
            $this->db->query("UPDATE tblausencias set tipo_ausencia='$modtipolicencia', observacion='$moddescripcion', desde='$moddesde', hasta='$modhasta', fechamod='$fechamod'
    ...
    public function getdatospersonal(){
        ...
        $area=$getarea[0];
        return $this->db->query_select("select rut,nombres from tblpersonal where id_area='$area' and estado=1 order by nombres");
    }
    // Obtiene Datos de ausencia para modifcar
    // OK JJARA
    public function get_ausencia_byId(int $id){
        return $this->db->query_select("select tblausencias.*, tblpersonal.nombres from
        tblausencias inner join tblpersonal on tblausencias.rut=tblpersonal.rut where id_tblausencias='$id' limit 1");
    }
    // Obtiene Area de usuario asociado a tabla personal
    // OK JJARA
    public function getarea() : array{
        ...
        $dato3=$this->db->query_select("select tblareas.id_area from tblareas inner join tblpersonal on tblareas.id_area=tblpersonal.id_area inner join users on tblpersonal.rut=users.rut_personal where users.id_user='$dato2'");
        if ($dato3 == false)
            return array('id_area' => '0');
        else
            return $dato3[0];
    }
    ...
    public function mostrar_datos(){
        ...
        $datreg= $this->getarea();
        $dato4=$datreg['id_area'];
        
        return $this->db->query_select("select id_tblausencias,a.rut,tipo_ausencia,desde,hasta,fechamod,u.name,p.nombres,usu_registra,observacion from (tblausencias a inner join tblpersonal p on a.rut=p.rut) inner join users u on a.usu_registra=u.id_user where a.id_area='$dato4' order by id_tblausencias desc limit 60");
    }
    ...
    public function buscar_rut(){
        ...
        $datasoc2=$this->getarea();
        $datasoc=$datasoc2[0];

        $dat2=$this->db->query_select("select rut,nombres from tblpersonal where id_area='$datasoc' and (rut like '$rut%' || nombres like '%$rut%') limit 1 ");
        if ($dat2 == false){
            return array('success' => 1, 'message' => 'Datos no encontrados');
        }else{
            return array('success' => 0, 'message' => $dat2[0]['nombres'], 'message1' => $dat2[0]['rut']);
        }
    }
    ...
    public function revisar_por_fecha(){
        ...
        $area=$http->request->get('textoarea');
        
        $usucompa = (new Model\Users)->getOwnerUser();
        
        $datosfecha=$this->db->query_select("select @rownum:=@rownum+1 as rownum,id_tblausencias,a.rut,tipo_ausencia,desde,hasta,fechamod,u.name,p.nombres,usu_registra from (select @rownum:=0) r,(tblausencias a inner join tblpersonal p on a.rut=p.rut)inner join users u on a.usu_registra=u.id_user where desde between '$fechadesde' and '$fechahasta' and a.id_area='$area'");
        ...
        if ($datosfecha === false){
            ...
            foreach ($datosfecha as $key => $value) {
                ...
                $json['aaData'][] = array($value['rownum'],$value['rut'],$value['nombres'],$value['tipo_ausencia'],$value['desde'],$value['hasta'],$value['name'],$value['fechamod'], $html );
            }
    ...
    public function exporta_excel_ausencias() {
        ...
        $area=$http->query->get('area');
        $u = $this->db->query_select("select a.rut,tipo_ausencia,desde,hasta,p.nombres,observacion from tblausencias a inner join tblpersonal p on a.rut=p.rut where a.id_area='$area' and desde between '$desde' and '$hasta'");
